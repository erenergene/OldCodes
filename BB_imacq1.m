% BB_imacq1.m - Breadboard for Image Acquisition: 1
...
preTall_B = matfile(imsB{1,1},'Writable',true);
preTall_G = matfile(imsG{1,1},'Writable',true);
preTall_R = matfile(imsR{1,1},'Writable',true);

%%

% preTall_B.array(:,:,2) = load(imsB{2,1}).array;
% preTall_G.array(:,:,2) = load(imsG{2,1}).array;
% preTall_R.array(:,:,2) = load(imsR{2,1}).array;

%%

for i = 2:nfiles
preTall_B.array(:,:,i) = load(imsB{i,1}).array;
preTall_G.array(:,:,i) = load(imsG{i,1}).array;
preTall_R.array(:,:,i) = load(imsR{i,1}).array;
end
%%
% preTall_R.array(:,:,2) = matfile(imsR{2,1});
