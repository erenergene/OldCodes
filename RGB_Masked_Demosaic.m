% RGB_Masked_Demosaic.m - Color Camera Image Acquisitions
...
R_Mask = uint16(repmat([1 0 ; 0 0],540,720));
G_Mask = uint16(repmat([0 1 ; 1 0],540,720));
B_Mask = uint16(repmat([0 0 ; 0 1],540,720));
...
%%
R_im = (RLight_Tiff - (RLeak_for_BLight + RLeak_for_GLight)).*R_Mask;
figure(5)
imshow(R_im)

G_im = (GLight_Tiff - (GLeak_for_RLight + GLeak_for_BLight)).*G_Mask;
figure(6)
imshow(G_im)

B_im = (BLight_Tiff - (BLeak_for_RLight + BLeak_for_GLight)).*B_Mask;
figure(7)
imshow(B_im)
%%
