% RGB_Masked_Demosaic_Eren.m - Color Camera Image Acquisitions
...
[R_Tiff_0nm_crop,x_crop0,y_crop0] = AreaSelection(R_Tiff_0nm);
G_Tiff_0nm_crop = G_Tiff_0nm(x_crop0:x_crop0+63,y_crop0:y_crop0+64);
B_Tiff_0nm_crop = B_Tiff_0nm(x_crop0:x_crop0+63,y_crop0:y_crop0+64);

[R_Tiff_Xnm_crop,x_cropX,y_cropX] = AreaSelection(R_Tiff_Xnm);
G_Tiff_Xnm_crop = G_Tiff_Xnm(x_cropX:x_cropX+63,y_cropX:y_cropX+64);
B_Tiff_Xnm_crop = B_Tiff_Xnm(x_cropX:x_cropX+63,y_cropX:y_cropX+64);

%once duplicate variable olustur rgb_tiff_xnm_masked for all


%Intensity In calculation (calibration) I_in = I_out_Si / Reflectance

I_inR = round(double(R_Tiff_0nm) ./ Ref_at_0nm_R);
I_inG = round(double(G_Tiff_0nm) ./ Ref_at_0nm_G);
I_inB = round(double(B_Tiff_0nm) ./ Ref_at_0nm_B);
...
RefRed_at_XnmO2 = (double(R_Tiff_Xnm)./I_inR).*double(R_Mask);
RefGre_at_XnmO2 = (double(G_Tiff_Xnm)./I_inG).*double(G_Mask);
RefBlu_at_XnmO2 = (double(B_Tiff_Xnm)./I_inB).*double(B_Mask);
