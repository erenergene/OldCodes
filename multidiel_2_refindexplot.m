...
NS_table = readtable('Si.csv')
NS = table2array(NS_table)
NSO2_table = readtable('SiO2.csv')
NSO2 = table2array(NSO2_table)
NS= NS'
NSO2 = NSO2'
...
%L= [li]
%L = linspace((lambda1(:,1)/10, (lambda1()/10, 41);
L = lambda1/10;
Z = [];
Gamma = [];

[Gamma, Z] = multidiels([NS(2,:); NSO2(2,:) ; ones(1,41)],L,lambda1);

plot(lambda1, Gamma)
cb = colorbar
cb.Location = 'southoutside';

function [Gamma,Z] = multidiels(n,L,lambda,theta,pol)
